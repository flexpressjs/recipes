module.exports = {
    ...
    /*
     * Express CORS middleware configuration (read more on https://github.com/expressjs/cors)
     */
    cors: {
         ...
         *
         *     - `Boolean` - set to `true` to reflect the request origin (`req.header('Origin')`) or
         *       set it to `false` to disable CORS.
         *
         *     - `String` - set to a specific origin. For example if you set it to `"http://example.com"`
         *       only requests from "http://example.com" will be allowed.
         *
         *     - `RegExp` - set to a regular expression pattern which will be used to test the request origin.
         ...
         *
         *     - `Array` - set to an array of valid origins. Each origin can be a `String` or a `RegExp`.
         ...
         *
         *     - `Function` - set to a function implementing some custom logic. The function takes the
        ...
        /*
         * Provides a status code to use for successful `OPTIONS` requests, since some legacy browsers
